...
allocatable dvan(:,:),vvv(:,:),v1tmp(:,:),v2tmp(:,:),dvapr(:,:),moho(:,:)
...
! -----------------------------
! FOR Moho depth calculation:
real all_long(500),all_lat(500), long, lat
!real long, lat
integer silo,nsilo,sila,nsila
! -----------------------------
...
close(1)
!******************************************************************
!****************************** Reads in Moho model that was used ************************************
open(1,file='../../DATA/'//re//'/'//ar//'/INI_PARAM/major_param.dat')
do i=1,10000
...
!*******************************************************************
! get Moho depths at different long/lats: (Sofia, 14.2.13)
! im file moho_coordinates.dat sind die Koordinaten von denen ich die Moho-Tiefe haben will 
! wenn all das berechnet werden soll, setze calc_m auf 1!

calc_m=1
if (calc_m.eq.1) then 
open(1,file='moho_coordinates.dat')
nsilo=0
do
read(1,*,end=200) 
nsilo=nsilo+1
...
open(1,file='moho_coordinates.dat')
open(2,file='moho_coordinates_depth.dat')
do silo=1,nsilo
	read(1,*)all_long(silo),all_lat(silo)
	write(*,*)' long ', all_long(silo)
	write(*,*)' lat', all_lat(silo)
	write(*,*)' Moho=',depth_moho(all_long(silo),all_lat(silo))
	write(2,*)all_long(silo),all_lat(silo),depth_moho(all_long(silo),all_lat(silo))
end do
close(1)
close(2)
end if
...
! get depths of the horicontal slices that shall be plotted
...
! if an a-priori initial model or an 3D model was pre-defined, this is written in here
...
! uses info from sethor.dat file to define area where tomo grids should be created
! get number of nodes to allocate memory for the different variables
rsmth=ismth+0.5
nfmap=int_best((fmap2-fmap1)/dfmap+1.)
ntmap=int_best((tmap2-tmap1)/dtmap+1.)
write(*,*)' nfmap=',nfmap,' ntmap=',ntmap
allocate(dvan(nfmap,ntmap),vvv(nfmap,ntmap),v1tmp(nfmap,ntmap),v2tmp(nfmap,ntmap),dvapr(nfmap,ntmap),moho(nfmap,ntmap))
...
do ips=1,2
	...
	DO ilev=1,nlev
		...
		do ifi=1,nfmap
			fffs=(ifi-1)*dfmap+fmap1+fi0
			do itet=1,ntmap
				ttts=(itet-1)*dtmap+tmap1+tet0
				!vanm=-999.
				!if (vvv(ifi,itet).gt.w_limit) then
				!	vanm=dvan(ifi,itet)/vvv(ifi,itet)
				!end if
				!call decsf(xcur,ycur,0.,fi0,tet0,fff,ttt,h)
				!moho(ifi,itet)=depth_moho(75.07417,37.57569)
				moho(ifi,itet)=-depth_moho(fffs,ttts)
				!write(*,*)' Moho =',moho(ifi,itet), ' ifi =',ttts, ' itet =',fffs
			end do
		end do
		...
		close(14)
               ...
               open(14,file='../../FIG_FILES/HOR/m'//ps//'_'//lv//'.grd')
		write(14,'(a4)')dsaa
		write(14,*)nfmap,ntmap
		write(14,*)fmap1+fi0,fmap2+fi0
		write(14,*)tmap1+tet0,tmap2+tet0
		write(14,*)-999,999
		do itet=1,ntmap
			write(14,*)(moho(ifi,itet),ifi=1,nfmap)
		end do
		close(14)
